...
import os
import csv
from Model.load_data import load_valid_samples, ValidDataset


logger = logging.getLogger(__name__)

class MyCERerankingEvaluator:
    """
    ...
    """
    def __init__(self, valid_samples_dir: str = "data", mrr_at_k: int = 10, name: str = '', write_csv: bool = True):
        # self.samples = samples
        self.valid_samples = load_valid_samples(valid_samples_dir)
        self.name = name
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        num_positives = []
        num_negatives = []
        for key, instance in self.valid_samples.items():
            ...
            # use my valid dataset
            cur_valid_dataset = ValidDataset(instance)
            pred_scores = model.predict(cur_valid_dataset, convert_to_numpy=True, show_progress_bar=False)
